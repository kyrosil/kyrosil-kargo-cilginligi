// --- Oyun Değişkenleri ---
let gleen;
let kargoPool = []; // Nesne Havuzu Dizisi
let score = 0; let misses = 0; let giftMessage = ''; let gameOver = false; let lives = 3;
...
const MAX_KARGOS = 60; // <<<--- EKSİK OLAN SATIR BURAYA EKLENDİ!
...
function triggerConfetti() { /* ... öncekiyle aynı ... */ if (typeof confetti === 'function') { console.log("BOMBA GİBİ KONFETİ!"); confetti({ particleCount: 150, spread: 90, origin: { y: 0.6 } }); setTimeout(() => { confetti({ particleCount: 100, angle: 60, spread: 75, origin: { x: 0.1, y: 0.7 } }); confetti({ particleCount: 100, angle: 120, spread: 75, origin: { x: 0.9, y: 0.7 } }); }, 150); } else { console.warn("Konfeti kütüphanesi yüklenemedi."); } }
...
function preload() { /* ... (öncekiyle aynı) ... */ try { trendyolLogo = loadImage('images.jpg'); kyrosilLogo = loadImage('cropped-adsiz_tasarim-removebg-preview-1.png'); } catch (e) { console.error('Logo yükleme hatası:', e); trendyolLogo = null; kyrosilLogo = null; } try { soundFormats('mp3', 'wav'); console.log("Ses dosyaları yükleniyor (Ana Dizin)..."); bgMusic = loadSound('Trendyol Yolla Şarkı Sözleri.mp3', soundLoaded, soundLoadError, soundLoadProgress); catchSound = loadSound('collect-points-190037.mp3', soundLoaded, soundLoadError, soundLoadProgress); missSound = loadSound('pickup-sound-82314.mp3', soundLoaded, soundLoadError, soundLoadProgress); gameOverSound = loadSound('game-over-arcade-6435.mp3', soundLoaded, soundLoadError, soundLoadProgress); clickSound = loadSound('Tık Sesi Efekti.mp3', soundLoaded, soundLoadError, soundLoadProgress); winSound = loadSound('you-win-sequence-2-183949.mp3', soundLoaded, soundLoadError, soundLoadProgress); } catch (e) { console.error("Ses yükleme başlatılırken hata oluştu:", e); bgMusic = catchSound = missSound = gameOverSound = clickSound = winSound = null; } }
...
function setup() {
    ...
    // Dikkat: playerWidth burada tanımlı olmalı veya globalden gelmeli
    gleen = { x: canvasW / 2 - playerWidth / 2, y: gleenY, w: playerWidth, h: playerHeight };

    kargoPool = []; // Havuzu temizle
    // <<<--- Havuzu oluştururken MAX_KARGOS KULLANILIYOR ---
    for (let i = 0; i < MAX_KARGOS; i++) {
        kargoPool.push({ active: false, x: 0, y: 0, w: 0, h: 0, speed: 0, isBonus: false });
    }
    console.log(MAX_KARGOS + " kapasiteli kargo havuzu oluşturuldu.");
    // --- Bitti ---
