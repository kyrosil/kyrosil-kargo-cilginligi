...
function updateStoredLives(newLives) { lives = newLives >= 0 ? newLives : 0; /* console.log("Hak güncellendi (değişken):", lives); */ }
...
function restartGame() {
    ...
    try { playSound(clickSound); if (lives > 0) { updateStoredLives(lives - 1); /* console.log("[restartGame] Hak düşürüldü. Yeni hak:", lives); */ if (lives > 0) { /* console.log("[restartGame] Hak kaldı, oyun sıfırlanıyor ve başlatılıyor..."); */ document.getElementById('restartButton').style.display = 'none'; document.getElementById('message').style.display = 'none'; resetGame(); if (gameInstanceCanvas) { gameInstanceCanvas.style('pointer-events', 'auto'); } frameCount = 0; if (bgMusic && bgMusic.isLoaded() && !isBgMusicPlaying) { bgMusic.loop(); isBgMusicPlaying = true; } else if (isBgMusicPlaying && bgMusic && !bgMusic.isPlaying()){ bgMusic.loop(); } loop(); /* console.log('[restartGame] Oyun başarıyla yeniden başlatıldı.'); */ } else { /* console.log('[restartGame] Son hak kullanıldı, oyun bitiyor.'); */ finalScore = score; gameOver = true; playSound(gameOverSound, 0.7); redraw(); } } else { console.warn("[restartGame] Hak yokken yeniden başlatma denendi!"); } } catch(restartError) { console.error("[restartGame] HATA oluştu!", restartError); }
}

function resetGame() {
    // console.log("[resetGame] Başladı.");
    try { score = 0; misses = 0; gameOver = false; finalScore = 0; confettiFired = false; for (let i = 0; i < kargoPool.length; i++) { kargoPool[i].active = false; } if (gleen) { gleen.w = playerWidth; gleen.x = width / 2 - gleen.w / 2; gleen.y = height - (isVertical ? 40 : 60); } const msgEl = document.getElementById('message'); if(msgEl) msgEl.style.display = 'none'; const restartBtnEl = document.getElementById('restartButton'); if(restartBtnEl) restartBtnEl.style.display = 'none'; /* console.log("[resetGame] Bitti."); */ } catch(resetError) { console.error("[resetGame] HATA oluştu!", resetError); }
}
