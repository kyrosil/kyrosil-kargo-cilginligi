...
let trendyolLogo = null, kyrosilLogo = null; // Başlangıçta null
...
const bonusSpawnChance = 0.10; // Bonus çıkma ihtimali %10
...
function checkLives() { return 3; } // localStorage devre dışı
...
function preload() {
    console.log("--- Preload Başladı ---");
    try {
        // !!! REİS: LÜTFEN DOSYA YOLU/ADINI KONTROL ET !!!
        console.log("Logo resimleri yükleniyor...");
        trendyolLogo = loadImage('images.jpg'); // <<<--- KONTROL ET!
        kyrosilLogo = loadImage('cropped-adsiz_tasarim-removebg-preview-1.png'); // <<<--- KONTROL ET!
        console.log("Logo yükleme komutları çalıştı.");
    } catch (e) { console.error('!!! Logo yüklenirken KRİTİK HATA:', e); trendyolLogo = null; kyrosilLogo = null; }
    try {
        soundFormats('mp3', 'wav');
        console.log("Ses dosyaları yükleniyor...");
         // !!! REİS: LÜTFEN DOSYA YOLU/ADINI KONTROL ET !!!
        bgMusic = loadSound('Trendyol Yolla Şarkı Sözleri.mp3');       // <<<--- KONTROL ET!
        catchSound = loadSound('collect-points-190037.mp3');           // <<<--- KONTROL ET!
        missSound = loadSound('pickup-sound-82314.mp3');              // <<<--- KONTROL ET!
        gameOverSound = loadSound('game-over-arcade-6435.mp3');         // <<<--- KONTROL ET!
        clickSound = loadSound('Tık Sesi Efekti.mp3');                 // <<<--- KONTROL ET!
        winSound = loadSound('you-win-sequence-2-183949.mp3');         // <<<--- KONTROL ET!
        console.log("Ses yükleme komutları çalıştı.");
    } catch (e) { console.error("!!! Ses yükleme hatası:", e); bgMusic = catchSound = missSound = gameOverSound = clickSound = winSound = null; }
    console.log("--- Preload Bitti ---");
}
...
function setup() {
    ...
    try {
        ...
        if (langTRButton) { langTRButton.addEventListener('click', () => { playSound(clickSound); if (currentLang !== 'TR') { currentLang = 'TR'; updateTexts(currentLang); } }); } else { throw new Error("TR Dil butonu bulunamadı!"); }
        if (langENButton) { langENButton.addEventListener('click', () => { playSound(clickSound); if (currentLang !== 'EN') { currentLang = 'EN'; updateTexts(currentLang); } }); } else { throw new Error("EN Dil butonu bulunamadı!"); }
    ...
    } catch (setupError) { console.error("[setup] KURULUM SIRASINDA KRİTİK HATA!", setupError); alert("Oyun kurulurken bir hata oluştu. Lütfen sayfayı yenileyin."); }
}
...
function draw() {
    ...
    try {
        ...
        if (gameOver) { const reward = getReward(finalScore, currentLang); const t = texts[currentLang]; const messageEl = document.getElementById('message'); const restartButtonEl = document.getElementById('restartButton'); messageEl.style.display = 'none'; messageEl.className = ''; restartButtonEl.style.display = 'none'; if (reward && reward.amount) { fill(0, 0, 0, 150); rect(0, 0, width, height); let boxW = width * 0.8; let boxH = height * 0.6; let boxX = (width - boxW) / 2; let boxY = (height - boxH) / 2; stroke(200); fill(250); rect(boxX, boxY, boxW, boxH, 10); textAlign(CENTER, CENTER); textSize(isVertical ? 20 : 28); fill('#155724'); let messageText = `${t.winMessagePart1}${finalScore}${t.winMessagePart2}${reward.amount}${t.winMessagePart3}`; text(messageText, boxX + 20, boxY + 20, boxW - 40, boxH * 0.5 - 30); textSize(isVertical ? 12 : 14); fill(80); text(t.winInstructions, boxX + 20, boxY + boxH * 0.5 , boxW - 40, boxH * 0.5 - 30); if (!confettiFired) { playSound(winSound, 0.6); triggerConfetti(); confettiFired = true; } } else { messageEl.innerText = `${t.gameOverBase}\n${t.scoreLabel}${finalScore}`; messageEl.style.color = '#dc3545'; messageEl.style.display = 'block'; } if (lives <= 0) { let noLivesText = `<br><br><strong style="color: red; font-size: 1.1em;">${t.noMoreLives}</strong>`; if (!messageEl.innerHTML.includes(t.noMoreLives)) { messageEl.innerHTML += noLivesText; } messageEl.style.display = 'block'; } if (lives > 0) { restartButtonEl.style.display = 'block'; } if (gameInstanceCanvas) { gameInstanceCanvas.style('pointer-events', 'none'); } noLoop(); return; }

        // --- Oyun Devam Ediyor ---
        fill(playerColor); noStroke(); rect(gleen.x, gleen.y, gleen.w, gleen.h, 5);
        gleen.x = constrain(mouseX - gleen.w / 2, 0, width - gleen.w);

        // <<<--- YENİ AGRESİF ZORLUK AYARLARI ---
        let spawnRate = 50; let minSpeed = 3; let maxSpeed = 7; // Base
        if (score >= 150) { spawnRate = 22; minSpeed = 9; maxSpeed = 21; }       // Seviye 7
        else if (score >= 100) { spawnRate = 25; minSpeed = 8; maxSpeed = 19; } // Seviye 6
        else if (score >= 75) { spawnRate = 28; minSpeed = 7; maxSpeed = 17; }  // Seviye 5
        else if (score >= 50) { spawnRate = 33; minSpeed = 6; maxSpeed = 15; }  // Seviye 4
        else if (score >= 25) { spawnRate = 38; minSpeed = 5; maxSpeed = 12; }  // Seviye 3
        else if (score >= 10) { spawnRate = 45; minSpeed = 4; maxSpeed = 9; }   // Seviye 2
        // --- Zorluk Bitti ---
